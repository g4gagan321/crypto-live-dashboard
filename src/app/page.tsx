...
import { ContentModeButton } from '@/components/ContentModeButton';
import { ErrorBoundary } from '@/components/common/ErrorBoundary';

// Single-screen, no-scroll, 16:9-optimized broadcast layout:
//   Header (fixed height)
//   Extras strip (fixed height): world clocks + weather
//   Signal strip (fixed height): "what matters now" — risk regime, VIX
//     band, breadth tilt, FII/DII net, biggest mover (derived, no new data)
...
export default function DashboardPage() {
  return (
    <ObsModeProvider>
      <main className="grid h-screen w-screen grid-rows-[56px_24px_26px_72px_1fr_24px_40px] gap-1.5 bg-terminal-bg p-1.5">
        ...
        <ErrorBoundary label="SIGNAL STRIP">
          <SignalStrip />
        </ErrorBoundary>
        ...
        <ErrorBoundary label="MAIN BODY">
          <div className="grid h-full min-h-0 grid-cols-[300px_620px_1fr] gap-1.5">
